...
Задержки можно переопределить переменными `KUFAR_QUERY_DELAY_SECONDS` и `KUFAR_LOOP_DELAY_SECONDS`. Значения Telegram из файла конфигурации служат только шаблоном и переопределяются переменными Railway.
...
- `➖ Удалить запрос` — показывает кнопки с текущими запросами и просит подтверждение перед удалением;
